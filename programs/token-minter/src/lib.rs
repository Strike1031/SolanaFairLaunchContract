...
mod token_minter {
    ...
    pub fn create_token(ctx: Context<InitToken>, metadata: InitTokenParams, amount: u64) -> Result<()> {

        require!(amount < ctx.accounts.global_info.initial_amount, CustomError::InvalidInitialValue);
        ...
        let token_price: u64 = get_price(ctx.accounts.token_pools.sol_reserve, ctx.accounts.token_pools.token_reserve);
        ...
        ctx.accounts.token_pools.token_reserve = ctx.accounts.global_info.total_supply - token_amount;
    ...
    pub fn buy_token(ctx: Context<BuyToken>, amount: u64) -> Result<()> {
        ...
        require!(ctx.accounts.token_pools.token_reserve > token_amount, CustomError::InvalidTokenAmount);
    ...
    pub fn sell_token(ctx: Context<SellToken>, token_amount: u64) -> Result<()> {
        ...
        require!(ctx.accounts.token_pools.sol_reserve > sol_amount, CustomError::InvalidSolAmount);
        // Transfer SOL from contract account to seller
        **ctx.accounts.escrow_account.to_account_info().try_borrow_mut_lamports()? -= sol_amount;
    ...
    pub fn withdraw_balance(ctx: Context<WithdrawBalance>, amount: u64) -> Result<()> {
        require!(ctx.accounts.global_info.owner == ctx.accounts.admin.key(), CustomError::NotOwner);
